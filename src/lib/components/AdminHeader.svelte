...
<header class="bg-gray-800 text-white border-b border-gray-700">
	<nav class="container mx-auto px-4 py-3">
		<div class="flex items-center justify-between">
			<!-- Logo -->
			<a href="/admin/dashboard" class="flex items-center transition-transform hover:scale-105">
			...
			<div class="hidden md:flex items-center gap-4">
				<!-- Publish Work Button -->
				<a href="/admin/works?action=publish" class="flex items-center gap-2 px-4 py-2 bg-primary text-white hover:bg-primary-dark transition-all duration-300 rounded btn-press shadow-md hover:shadow-lg">
					<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
					</svg>
					<span style="font-family: 'Space Grotesk', sans-serif;">Publish Work</span>
				</a>
				
				<!-- User Profile -->
				<div class="flex items-center gap-3">
					{#if user?.avatar_url}
						<img src={user.avatar_url} alt={user.name || 'Admin'} class="w-10 h-10 rounded-full border-2 border-gray-600" />
					{:else}
					...
					{/if}
					<span class="text-sm font-semibold" style="font-family: 'Space Grotesk', sans-serif;">{user?.name || 'Admin'}</span>
					...
					<div class="relative">
						<button
							...
							class="p-1 hover:bg-gray-700 rounded transition-colors duration-300"
						>
							<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
						...
						{#if showDropdown}
							...
							>
								<a href="/admin/settings" class="flex items-center gap-2 px-4 py-3 text-gray-700 hover:bg-gray-50 transition-all duration-300 border-b border-gray-200">
									<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
