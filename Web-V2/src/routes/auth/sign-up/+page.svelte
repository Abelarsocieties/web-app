<script lang="ts">
	...
	function hasOtpSent(data: unknown): boolean {
		if (!data || typeof data !== 'object') return false;
		return (data as Record<string, unknown>).otpSent === true;
	}
		...
		<div class="w-full max-w-md animate-fade-in">
			<div class="text-center mb-8">
				...
				<p class="text-gray-500" style="font-family: 'Space Grotesk', sans-serif;">
					Welcome to Abelar! Request a code, then create your account.
				</p>
			</div>
			...
			<form
				...
				use:enhance={() => {
					...
					return async ({ result, update }) => {
						...
						if (result.type === 'success' && hasOtpSent(result.data)) {
							otpSent = true;
						}
						await update();
					};
			...
			>
				...
				{#if otpSent}
					<div
						class="p-4 bg-emerald-50 border border-emerald-200 text-emerald-800 text-sm rounded"
						style="font-family: 'Space Grotesk', sans-serif;"
					>
						Verification code sent. Check your email, enter the code below, then create your account.
					</div>
				{/if}

				{#if showSocialError}
					<div
						class="p-4 bg-amber-50 border-2 border-amber-300 text-amber-800 text-sm transition-all duration-300 rounded animate-fade-in"
						role="alert"
					>
				...
				<div>
					<label for="otp" class="block text-sm font-medium text-gray-700 mb-1.5" style="font-family: 'Space Grotesk', sans-serif;">Verification code</label>
					<input
						id="otp"
						name="otp"
						type="text"
				...
				<button
					...
					formaction="?/sendOtp"
					disabled={loading}
					class="w-full px-6 py-3 border-2 border-primary text-primary hover:bg-primary hover:text-white transition-all duration-300 font-medium disabled:opacity-50 btn-press"
					style="font-family: 'Space Grotesk', sans-serif;"
				>
