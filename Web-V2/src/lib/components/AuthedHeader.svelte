<script lang="ts">
	...
	function handleClickOutside(event: MouseEvent) {
		if (
			userDropdownRef &&
			userButtonRef &&
			!userDropdownRef.contains(event.target as Node) &&
			!userButtonRef.contains(event.target as Node)
		) {
			showDropdown = false;
		}
	}
	...
	<nav class="container mx-auto px-4 py-3">
		<div class="flex items-center justify-between">
			...
			<div class="hidden md:flex items-center gap-4">
				<a
					href="/app/submit"
					class="flex items-center gap-2 px-4 py-2 bg-primary text-white hover:bg-primary-dark transition-all duration-300 rounded btn-press shadow-md hover:shadow-lg"
				>
				...
				<div class="flex items-center gap-3">
					...
					{:else}
						<div
							class="w-10 h-10 rounded-full bg-gray-600 border-2 border-gray-500 flex items-center justify-center"
						>
				...
				<div class="relative">
					...
					{#if showDropdown}
						...
						>
							<a
								href="/app/settings"
								class="flex items-center gap-2 px-4 py-3 text-gray-700 hover:bg-gray-50 transition-all duration-300 border-b border-gray-200"
							>
								<svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path
										stroke-linecap="round"
